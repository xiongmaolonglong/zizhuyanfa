      ...
      <el-tab-pane label="部门管理" name="departments">
        <div class="tab-content">
          <div class="toolbar mb-16">
            <el-button type="primary" @click="openDeptDialog()">+ 创建部门</el-button>
          </div>
      ...
      <el-tab-pane label="人员管理" name="users">
        <div class="tab-content">
          <div class="toolbar mb-16">
            ...
            <el-select v-model="userFilter.role" placeholder="全部角色" clearable style="width:120px" class="mr-8" @change="fetchUsers">
          ...
          <el-table :data="users" stripe v-loading="userLoading" @selection-change="handleUserSelection">
            ...
            <el-table-column label="角色" width="120">
              <template #default="{ row }"><el-tag size="small" type="info">{{ roleLabel(row.role) }}</el-tag></template>
            </el-table-column>
          ...
          <el-pagination v-if="userTotal > userFilter.limit" class="mt-16" layout="prev, pager, next" :total="userTotal" :page-size="userFilter.limit" :current-page="userFilter.page" @current-change="handleUserPageChange" />
...
const ROLE_MAP = {
  ...
  designer: '设计师', producer: '生产管理员', constructor: '施工队长', finance: '财务',
}
...
async function fetchDepts() {
  ...
  try {
    const res = await api.get('/tenants/departments')
...
async function handleSaveDept() {
  ...
  try {
    if (editingDept.value) {
      await api.put(`/tenants/departments/${editingDept.value.id}`, deptForm)
...
async function handleDeleteDept(row) {
  try {
    ...
    await api.delete(`/tenants/departments/${row.id}`)
...
function handleUserSelection(rows) { selectedUsers.value = rows }
...
async function batchDelete() {
  ...
  try {
    ...
    for (const user of selectedUsers.value) {
      await api.delete(`/tenants/users/${user.id}`)
    }
...
async function fetchUsers() {
  ...
  try {
    ...
    const res = await api.get('/tenants/users', { params })
...
async function handleResetPassword(row) {
  try {
    ...
    await api.put(`/tenants/users/${row.id}`, { password: value })
...
async function handleDeleteUser(row) {
  try {
    ...
    await api.delete(`/tenants/users/${row.id}`)
